...
int TCorrelator::Correlate(TBetaDecayParams &bdecay, TBetaDecayVariables &bdecayv)
{
  ...
  if(isImplant)
	...
	{ // We have a good position
	  ...
	  //9/2
	  double tdiff_sega[16] = {-1};
	  double tdiff_labr3[15] = {-1};
	  if(fImplant[xPos][yPos].time >= 0)
		...
		tdiff_sega[det] = bdecay.sega.time[det]-fImplant[xPos][yPos].time;
	      }
	      for(int det = 0; det < 15; det ++){
		tdiff_labr3[det] = bdecay.labr3.time[det]-fImplant[xPos][yPos].time;
	      }
	      if(timeDiffIon > 0)
		{	    
  ...
  //else if(isCloverOnly)
    ...
    { // Can do some isomer hunting in here
      ...
      // 	  for(int det = 0; det < 16; det ++){
      // 	    tdiff[det] = bdecay.sega.time[det]-fImplant[xg][yg].time;
      // 	    bdecay.corr.sega_implant_tdiff[det] = tdiff[det];
      // 	  }
      // 	}
      // }  
    }
    ...
    { // Can do some isomer hunting in here
      ...
      // 	for(int yg = 0; yg < npspmt; yg++){
      // 	  for(int det = 0; det < 15; det ++){
      // 	    tdiff[det] = bdecay.labr3.timecal[det]-fImplant[xg][yg].time;//time aligned labr3s
      // 	    //if(tdiff[det] < maxtdiff){
      // 	    // gtime[xg][yg] = tdiff[det];//gamma of individual implants that have their specific positions
      // 	    bdecay.corr.labr3_implant_tdiff[det] = tdiff[det];//gamma of individual implants that have their specific positions
