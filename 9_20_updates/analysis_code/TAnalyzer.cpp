...
void TAnalyzer::AnalyzeData(TTree* tree_in, TTree* tree_out, std::string runnumber) 
{
  ...
  //for (auto evt=74; evt<nevents; evt++)
    //for (auto evt=0; evt<0.1*nevents; evt++)
    {
      ...
      // 	break;
      // }
      // Check with only 50% of the file
      // if(evt > 0 && (evt % (int)(nevents * 0.5)) == 0) {
      // 	break;
      // }  
      /**************************************/
      ...
      if(corr.Correlate(bdecay,bdecayv))
	{
      ...
      // 		// cout<<"evt: "<<evt<<" sega_"<<det<<"_ecal: "<<bdecay.sega.ecal[det]<<" xPos: "<<xg<<" yPos: "<<yg<<" sega_time: "<<bdecay.sega.time[det]<<" dynode_time: "<< bdecay.pspmt.dytime <<" sega_dy_tdiff: "<<bdecay.corr.sega_dynode_tdiff[det][xg][yg]<<endl;
      // 		//pspmt.dytime = 0 inside sega logic.
      // 		cout<<"evt: "<<evt<<" sega_"<<det<<"_ecal: "<<bdecay.sega.ecal[det]<<" xPos: "<<xg<<" yPos: "<<yg<<" sega_time: "<<bdecay.sega.time[det] <<" sega_dy_tdiff: "<<bdecay.corr.sega_dynode_tdiff[det][xg][yg]<<endl;
      // 		//}
      // 	      }
      // 	    }
      // 	  }
