 ...
 * 80 - good pos (isomer search)        *
 * 84 - bad pos (isomer search)         *
...
//Creating new implant search (for isomer hunting)
//TO 09/12/22
void TImplantIsomer::Reset()
{
  time = 0;        // Time of event
  dE1 = 0;         // dE from Pin01
  tofpin01i2s = 0; // Time of flight Pin01-I2S
...
void TSegaImplant::Reset()
{
  dt = 0;          // Time since last correlated implant 
  time = 0;        // Time of event
}

void TLabr3Implant::Reset()
{
  dt = 0;          // Time since last correlated implant 
  time = 0;        // Time of event
}
...
int TCorrelator::Correlate(TBetaDecayParams &bdecay, TBetaDecayVariables &bdecayv)
{
  ...
  bool isImplantIsomer = false; // isomer search
	...
	if(hasPin01Imp && hasPin02Imp && hasPSPMT)
	  {
	    ...
	    bdecay.corr.isimplant = 1;
	    // TO 09/12/22
	    isImplantIsomer = true;
	    bdecay.corr.isimplantisomer = 1;
	  }
      }
      //}
    }
  ...
  if(!hasPin && !hasPSPMT && hasLaBr3)
    {
      isLaBr3Only = true; //rework name conventions
    }
  ...
  if(isImplant)
	...
	{ // We have a good position
	  ...
	  //gImplant entries TO 09/12/22
	  //We don't want this to depend on current_time // bdecay.clock.initial //Or if we do; consider to change unit to ns and put inside fImplant.time condition
	  //gimplant time >=0 condition...
	  ...
	  bdecay.corr.gtime = gImplant[xPos][yPos].time;
	  bdecay.corr.gde1 = gImplant[xPos][yPos].dE1;
	  bdecay.corr.gtofpin01i2s = gImplant[xPos][yPos].tofpin01i2s;

	  // bdecay.corr.sde1  = gImplant[xPos][yPos].dE1;
	  // bdecay.corr.stofpin01i2s =  gImplant[xPos][yPos].tofpin01i2s;
	  // bdecay.corr.lde1  = gImplant[xPos][yPos].dE1;
	  // bdecay.corr.ltofpin01i2s =  gImplant[xPos][yPos].tofpin01i2s;

	  //SeGAs
	  // //Position dependent
	  double tdiff_sega[nsega][npspmt][npspmt];
	  //Energy condition
	  for(int det = 0; det < nsega; det ++){
		...
		for(int yPos = 0; yPos < npspmt; yPos++){
		  ...
		  bdecay.corr.sega_dynode_tdiff[det][xPos][yPos] = tdiff_sega[det][xPos][yPos];
		  bdecay.corr.sega_dynode_itofpin01i2s[det][xPos][yPos] = gImplant[xPos][yPos].tofpin01i2s;
		  bdecay.corr.sega_dynode_idE1[det][xPos][yPos] = gImplant[xPos][yPos].dE1;
		}
	      }
	    }
	  }
	  ...
	  //Labr3s
	  // //Position dependent
	  double tdiff_labr3[nlabr3][npspmt][npspmt];
	  //Energy condition
	  for(int det = 0; det < nlabr3; det ++){
		...
		for(int yPos = 0; yPos < npspmt; yPos++){
		  ...
		  bdecay.corr.labr3_dynode_tdiff[det][xPos][yPos] = tdiff_labr3[det][xPos][yPos];
		  bdecay.corr.labr3_dynode_itofpin01i2s[det][xPos][yPos] = gImplant[xPos][yPos].tofpin01i2s;
		  bdecay.corr.labr3_dynode_idE1[det][xPos][yPos] = gImplant[xPos][yPos].dE1;
		}
	      }
	    }
	  }
  ...
  //else if(isCloverOnly)
	...
	if(bdecay.sega.ecal[det] > 10){
	  for(int xg = 0; xg < npspmt; xg++){
	    for(int yg = 0; yg < npspmt; yg++){
	      tdiff_sega[det][xg][yg] = bdecay.sega.time[det]-gImplant[xg][yg].time;	  
	      bdecay.corr.sega_dynode_tdiff[det][xg][yg] = tdiff_sega[det][xg][yg];
	      bdecay.corr.sega_dynode_itofpin01i2s[det][xg][yg] = gImplant[xg][yg].tofpin01i2s;
	      bdecay.corr.sega_dynode_idE1[det][xg][yg] = gImplant[xg][yg].dE1;
	      //bdecay.corr.sde1  = gImplant[xg][yg].dE1;
	      //bdecay.corr.stofpin01i2s =  gImplant[xg][yg].tofpin01i2s;

	      //Cout minimum time difference; event num; put counter for events less than 200 us
   
	      // if(gImplant[xg][yg].tofpin01i2s!=0 && gImplant[xg][yg].dE1!=0){
	      // 	cout<<"gimplant de1: "<<gImplant[xg][yg].dE1 << " "<<bdecay.corr.sega_dynode_idE1[det][xg][yg]<<"gimplant tof :"<<gImplant[xg][yg].tofpin01i2s<< " "<<bdecay.corr.sega_dynode_itofpin01i2s[det][xg][yg]<<endl;
	      // }


	      //if(bdecay.corr.sega_dynode_tdiff[det][xg][yg] > 0){
	      // if(gImplant[xg][yg].time > 0){
	      // 	cout<< std::setprecision(12)<<" sega_"<<det<<"_ecal: "<<bdecay.sega.ecal[det]<<" xPos: "<<xg<<" yPos: "<<yg<<" sega_time: "<<bdecay.sega.time[det]<<" dynode_time: "<< gImplant[xg][yg].time <<" current_time: "<<current_time*1.e6 <<" sega_dy_tdiff: "<<bdecay.corr.sega_dynode_tdiff[det][xg][yg]<<endl;
	      // }

	      // if(fImplant[xg][yg].time > 0){
	      // 	cout<< std::setprecision(12)<<" sega_"<<det<<"_ecal: "<<bdecay.sega.ecal[det]<<" xPos: "<<xg<<" yPos: "<<yg<<" current_time: "<<current_time*1.e6 <<" fImplant_time: "<<fImplant[xg][yg].time*1.e9 <<" time_difference: " <<current_time*1.e6 - fImplant[xg][yg].time*1.e6 <<endl;
	      // }
	...
	if(bdecay.labr3.ecal[det] > 10){
	  for(int xg = 0; xg < npspmt; xg++){
	    for(int yg = 0; yg < npspmt; yg++){
	      tdiff_labr3[det][xg][yg] = bdecay.labr3.time[det]-gImplant[xg][yg].time;	  
	      bdecay.corr.labr3_dynode_tdiff[det][xg][yg] = tdiff_labr3[det][xg][yg];
	      bdecay.corr.labr3_dynode_itofpin01i2s[det][xg][yg] = gImplant[xg][yg].tofpin01i2s;
	      bdecay.corr.labr3_dynode_idE1[det][xg][yg] = gImplant[xg][yg].dE1;
	      bdecay.corr.lde1  = gImplant[xg][yg].dE1;
	      bdecay.corr.ltofpin01i2s =  gImplant[xg][yg].tofpin01i2s;
	    }
	  }
	}
      }    
